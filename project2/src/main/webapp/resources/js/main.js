...
const savaId = document.getElementById("saveId");

if(savaId != null){
    
    // radio, checkbox 값이 변할 때 발생하는 이벤트 : change
    savaId.addEventListener("change", function(event){
...
function loginValidate(){
    ...
    // document.querySelectio("css 선택자")
    ...
    if( memberPw.value.trim().length == 0 ){
        // 비밀번호가 양쪽 공백을 제거한 후 길이가 0인 경우
        // == 비밀번호가 미작성

        alert("비밀번호를 입력해주세요");

        memberPw.focus(); // 비밀번호가 input 요소에 초점을 맞춤
        memberPw.value = ""; // 비밀번호가 input 요소에 작성된 값을 모두 삭제 
