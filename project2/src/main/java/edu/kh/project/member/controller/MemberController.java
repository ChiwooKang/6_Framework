...
//	@RequestMapping(value = "/member/login", method = RequestMethod.POST)
	...
	// POST 방식의 /member/login 요청을 연결 @PostMapping("member/login))
	//@GetMapping("member/login") // GET  방식의  /member/login 요청을 연걸
	// == @RequestMapping(value="/member/login", method = RequestMethod.POST)
	
	// 3. @ModelAttribute 어노테이션 이용
	
	// [작성법]
	// @ModelAttribyte VO타입 매개변수명
	// -> 파라미터의 name속성 값이
	// 	  지정된 VO의 필드명과 같다면
